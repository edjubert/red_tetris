...
const clientGameMode =
	(io: Server, client: Client, room: Game | undefined, roomname: string) => (gameMode: string) => {
...
export const handleJoinRoom =
	...
	async ({ roomname, user }: { roomname: string; user: string }) => {
		...
		client.on(
			`${CLIENT_EVENTS.RESTART}:${roomname}`,
			clientRestart(io, conn, client, rooms, room, roomname)
		);
